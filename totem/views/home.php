...
*/
    ...
    }else if($win == 0)
    {
    	...
    	$request2  = 'GET /requests/status.xml?command=pl_play&id=8'.' HTTP/1.1' . "\r\n";
		$request2 .= 'Host: ' . $cfg['player_host'] . ':' . $cfg['player_port'] . "\r\n";
		$request2 .= 'Connection: Close' . "\r\n";
		$request2 .= 'Authorization: Basic ' . base64_encode(':' . $cfg['player_pass']) . "\r\n\r\n";
	...
	if($win =='LOSE')
	{
		  $request3  = 'GET /requests/status.xml?command=pl_play&id=9'.' HTTP/1.1' . "\r\n";
    ...
    if($win ==10)
	{
 	   ...
 	   $request3  = 'GET /requests/status.xml?command=pl_play&id=10'.' HTTP/1.1' . "\r\n";
    	$request3 .= 'Host: ' . $cfg['player_host'] . ':' . $cfg['player_port'] . "\r\n";
    	$request3 .= 'Connection: Close' . "\r\n";
    	$request3 .= 'Authorization: Basic ' . base64_encode(':' . $cfg['player_pass']) . "\r\n\r\n";
	 ...
	 if($win ==11)
	{
 	   ...
 	   $request3  = 'GET /requests/status.xml?command=pl_play&id=11'.' HTTP/1.1' . "\r\n";
    	$request3 .= 'Host: ' . $cfg['player_host'] . ':' . $cfg['player_port'] . "\r\n";
    	$request3 .= 'Connection: Close' . "\r\n";
    	$request3 .= 'Authorization: Basic ' . base64_encode(':' . $cfg['player_pass']) . "\r\n\r\n";
...
//echo("<pre>" . vlc2("pl_loop") . "</pre>");
	...
	{
		if($_GET['cliente'] != null || $_GET['cliente'] !='' )
		{
    ...
    {
		/*echo "SELECT * FROM ".newgiornata." WHERE `data` = DATE('$today') AND TIME('$ora') > `inizio` AND TIME('$ora') < `fine` LIMIT 1";
		die();*/
        if ($obj 	= $result->fetch_object())
		{
	...
	switch ($view)
	{
		...
		if($premioNome==='GIFT CARD 50')
		{
			...
			<body class='container'>
				...
				<video id='win' width='100%' autoplay>
					<source src='http://10.0.0.2/totem/media/win.mp4' type='video/mp4'>
		...
		$body = 
			...
			<body class='container'>
				...
				<div id='totemPremi'>$textPremi</div>
				<video id='win' width='100%' autoplay>
					<source src='http://10.0.0.2/totem/media/lose.mp4' type='video/mp4'>
					Il tuo browser non supporta i video HTML5.
				</video>
		...
		//	header('location:../' );
			...
			"
			<body>
				<img src='http://10.0.0.2/totem/media/credit.jpg' width='100%'>
		...
		case 'ERROR': 
			$body = "
			<body>
				<img src='http://10.0.0.2/totem/media/error.jpg' width='100%'>
			</body>
			<script> 
				setTimeout(function () {
   					window.location.href = '../';
